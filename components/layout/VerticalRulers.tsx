...
function Ruler({ column }: { column: "left" | "right" }) {
  ...
  return (
    <div
      className={`text-hex-foreground/35 ${dmMono.className} pointer-events-none fixed top-0 z-20 flex h-full w-8 flex-col justify-between border-hex-foreground/10 py-6 text-[9px] font-medium leading-none antialiased sm:w-9 sm:py-10 sm:text-[10px] md:w-10 ${position} ${column === "right" ? "items-end border-l" : "items-start border-r"}`}
      aria-hidden
    >
      {items.map((n, i) => (
